...
#define OLSR_MSG_HEADER_SIZE 21
...
uint32_t MessageHeader::GetMPRSerializedSize (void) const                  //Changed here
{
   uint32_t size = OLSR_MSG_HEADER_SIZE;
   switch (m_messageType)
   {
        case TC_MESSAGE:
          size += m_message.tc.GetMPRSerializedSize ();
        default:
          NS_ASSERT (false);
    }
   return size;
...
{
   //os << "The TLV type is " << m_TLV;  //Changed here
}
...
MessageHeader::Serialize (Buffer::Iterator start) const
{
  ...
  i.WriteU8 (m_TLV);   //Changed here
  i.WriteU8 (num_TLV);  //Changed here
  i.WriteHtonU32 (m_hoplimit);   //Changed here
  switch (m_messageType)
    {
    case MID_MESSAGE:
...
MessageHeader::Deserialize (Buffer::Iterator start)
{
 ...
 m_TLV = i.ReadU8 ();                        //Changed here
 num_TLV = i.ReadU8 ();                      //Changed here
 m_hoplimit = i.ReadNtohU32 ();              //Changed here
...
{
  Buffer::Iterator i = start;
  i.WriteU8 (this->TLVtype);  //Changed here
  i.WriteU8 (this->num_TLV);  //Changed here
...
MessageHeader::Hello::Deserialize (Buffer::Iterator start, uint32_t messageSize)
{
  ...
  this->num_TLV = i.ReadU8 ();    //Changed here
  this->TLVtype = i.ReadU8 ();    //Changed here
  helloSizeLeft -= 4;
...
uint32_t MessageHeader::Tc::GetMPRSerializedSize (void) const           //Changed here
{
  return 4 + this->mprselectoraddresses.size() * IPV4_ADDRESS_SIZE;
}
void
