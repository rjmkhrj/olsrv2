...
#include "ns3/olsr-header.h" //Changed here
...
struct InterfaceTuple   //Changed here
{
  Time time;
...
static inline bool operator == (const InterfaceTuple &a, const InterfaceTuple &b)   //Changed here
{
  return (a.senderAddress == b.senderAddress && a.mg == b.mg && a.seqNumber == b.seqNumber);
}

static inline std::ostream& operator << (std::ostream &os, const InterfaceTuple &tuple)  //Changed here
{
  os << "Originating Address" << tuple.senderAddress << " seqnumber " << tuple.seqNumber << " messagetype " << tuple.mg << " time " << tuple.time << ".";
return os;
}

struct ProcessedTuple   //Changed here
{
   Time time;
   Ipv4Address senderAddress;
   uint16_t seqNumber;
...
static inline bool operator == (const ProcessedTuple &a, const ProcessedTuple &b) //Changed here
{
  return (a.senderAddress == b.senderAddress && a.mg == b.mg && a.seqNumber == b.seqNumber);
}

static inline std::ostream& operator << (std::ostream &os, const ProcessedTuple &tuple)  //Changed here
{
  os << "Originating Address" << tuple.senderAddress << " seqnumber " << tuple.seqNumber << " messagetype " << tuple.mg << " time " << tuple.time << ".";
return os;
}
...
struct LinkTuple
{
  ...
  /// Time at which this tuple expires and must be removed.
  Time time;
  //linktype lt;             //Changed here
  enum sl               //Changed here
  {
    SY_M = 1,
    UNS_M = 2,
  } sll;
  
  enum out_metric       //Changed here
  {
    KN = 1,
    UNK = 2,
  } mtr;
...
struct NeighborTuple
{
  ...
  uint8_t willingness;
  //uint8_t MPRselectset;    //New Protocol
  uint8_t degree;   //Changed here (Degree)
  bool N_symmetric; //Changed here
};
...
struct TwoHopNeighborTuple
{
  ...
  uint8_t out_metric; //Changed here N2_out_metric
  uint8_t degree;   //Changed here
  uint8_t degree2;  //Changed here
};
...
typedef std::vector<InterfaceTuple>             InterfaceSet; //!<Interface set tuples    changed here
typedef std::vector<ProcessedTuple>             ProcessedSet; //!<Processed set tuples    changed here

struct SingleHop               //Changed here
{
   Ipv4Address mainAddr;
   uint8_t deg;
   uint8_t will;
   uint8_t degx2;
   TwoHopNeighborSet stt;                //vector of link tuples
};

static inline bool
operator == (const SingleHop &a, const SingleHop &b)             //Changed here
{
  return (a.mainAddr == b.mainAddr
          && a.deg == b.deg
          && a.degx2 == b.degx2);
...
struct DoubleHopN           //Changed here
{
   Ipv4Address mainAddr;
   uint8_t deg;
   uint8_t deg2;
...
static inline bool
operator == (const DoubleHopN &a, const DoubleHopN &b)
{
  return (a.mainAddr == b.mainAddr
          && a.deg == b.deg
          && a.deg2 == b.deg2);
...
static inline bool
operator == (const Comb &a, const Comb &b)
{
  return (a.main1 == b.main1
          && a.main2 == b.main2
          && a.d == b.d);
}

struct AllowedLinkTuple       //Changed here
{
   Ipv4Address neighaddr;
};

struct Rxm                    //Changed here
{
   Ipv4Address mainaddr;
   uint8_t count;
};
...
static inline bool
operator == (const Rxm &a, const Rxm &b)
{
  return (a.mainaddr == b.mainaddr
          && a.count == b.count);
}

struct Allowed2HopTuple       //Changed here
{
   Ipv4Address neighaddr;
};

static inline bool
operator == (const Allowed2HopTuple &a, const Allowed2HopTuple &b)
{
  return (a.neighaddr == b.neighaddr);
}


struct FloodingMPR            //Changed here
{
   Ipv4Address neighAddr;
};

struct MPRFloodingSelector        //Changed here
{
   Ipv4Address neighAddr;
};


typedef std::vector<SingleHop> singhop;   //Changed here
typedef std::vector<DoubleHopN> doublehop; //Changed here
typedef std::vector<AllowedLinkTuple> allowedtuples;   //Changed here
typedef std::vector<Allowed2HopTuple> allowed2hops;    //Changed here
typedef std::vector<Comb> dxy; //Changed here
typedef std::vector<Rxm> rxm;   //Changed here
typedef std::vector<FloodingMPR> fmpr;  //Changed here
typedef std::vector<MPRFloodingSelector> mfsp; //Changed here
