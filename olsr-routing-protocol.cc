...
/********** TLV types ***********/        //Changed here
//#define MPR_WILLING 1
//#define CONT_SEQ_NUM 2
...
/// Not neighbor type.
#define OLSR_NOT_NEIGH          2
/// Symmetric neighbor type.
#define OLSR_SYM_NEIGH          4
/// Asymmetric neighbor type.
#define OLSR_MPR_NEIGH          6
...
RoutingProtocol::SetIpv4 (Ptr<Ipv4> ipv4)
{
  ...
  m_midInterval.GetResolution ();

  m_packetSequenceNumber = OLSR_MAX_SEQ_NUM;
  uint16_t r = rand() % OLSR_MAX_SEQ_NUM;                 //Changed here
  m_messageSequenceNumber = r;                            //Changed here
  m_ansn = OLSR_MAX_SEQ_NUM;
  m_TLV = 1;                                              //Changed here
  m_linkTupleTimerFirstTime = true;
  uint16_t will = (rand() % 6) + 1;                             //Changed here
  m_willingness = will;                                   //CHanged here
  m_ipv4 = ipv4;
...
//         }
  ...
  for (MessageList::const_iterator messageIter = messages.begin ();
       messageIter != messages.end (); messageIter++)
    {
      ...
      MessageType mm; //Changed here
      if(messageHeader.GetMessageType () == olsr::MessageHeader::HELLO_MESSAGE)
          mm = HELLO_MESSAGE;
      if(messageHeader.GetMessageType() == olsr::MessageHeader::MID_MESSAGE)
          mm = MID_MESSAGE;
      if(messageHeader.GetMessageType() == olsr::MessageHeader::TC_MESSAGE)
          mm = TC_MESSAGE;
      if(messageHeader.GetMessageType() == olsr::MessageHeader::HNA_MESSAGE)
          mm = HNA_MESSAGE;
      
      InterfaceTuple* tt = m_state.FindInterfaceTuple (messageHeader.GetOriginatorAddress (), messageHeader.GetMessageSequenceNumber (), mm);  //Changed here*/
      
      if (tt == NULL)
      {
      InterfaceTuple intertup;    //Changed here
      intertup.senderAddress = messageHeader.GetOriginatorAddress (); //Changed here
      intertup.seqNumber = messageHeader.GetMessageSequenceNumber (); //Changed here
      if(messageHeader.GetMessageType () == olsr::MessageHeader::HELLO_MESSAGE)
          intertup.mg = HELLO_MESSAGE;
      if(messageHeader.GetMessageType() == olsr::MessageHeader::MID_MESSAGE)
          intertup.mg = MID_MESSAGE;
      if(messageHeader.GetMessageType() == olsr::MessageHeader::TC_MESSAGE)
          intertup.mg = TC_MESSAGE;
      if(messageHeader.GetMessageType() == olsr::MessageHeader::HNA_MESSAGE)
          intertup.mg = HNA_MESSAGE;
      
      m_state.InsertInterfaceTuple (intertup); //Changed here
      
      ProcessedTuple protup;     //Changed here
      protup.senderAddress = messageHeader.GetOriginatorAddress (); //Changed here
      protup.seqNumber = messageHeader.GetMessageSequenceNumber (); //Changed here
     if(messageHeader.GetMessageType () == olsr::MessageHeader::HELLO_MESSAGE)
          protup.mg = HELLO_MESSAGE;
      if(messageHeader.GetMessageType() == olsr::MessageHeader::MID_MESSAGE)
          protup.mg = MID_MESSAGE;
      if(messageHeader.GetMessageType() == olsr::MessageHeader::TC_MESSAGE)
          protup.mg = TC_MESSAGE;
      if(messageHeader.GetMessageType() == olsr::MessageHeader::HNA_MESSAGE)
          protup.mg = HNA_MESSAGE;

      m_state.InsertProcessedTuple (protup); //Changed here */
      // If ttl is less than or equal to zero, or
      // the receiver is the same as the originator,
      // the message must be silently dropped or there are more than one TLVs
      if ((messageHeader.GetTimeToLive () == 0
          || messageHeader.GetOriginatorAddress () == m_mainAddress)  || ((messageHeader.GetMessageType () != olsr::MessageHeader::HELLO_MESSAGE) && (messageHeader.GetMessageSequenceNumber () >65536 || messageHeader.GetMessageSequenceNumber ()<0)) || (messageHeader.GetNumTLV ()>1 && messageHeader.GetMessageTLV ()==1)) //changed here---------------------------------
        {
          packet->RemoveAtStart (messageHeader.GetSerializedSize ()
                                 - messageHeader.GetSerializedSize ());
          continue;
      ...
      if((messageHeader.GetMessageHopLimit ()<= 1) || (messageHeader.GetHopCount ()>= 255))   //Changed here
         do_forwarding = false;
      ...
      if (do_forwarding)
        {
          ...
          if (messageHeader.GetMessageType ()  != olsr::MessageHeader::HELLO_MESSAGE)
            {
              if (messageHeader.GetMessageType () != olsr::MessageHeader::TC_MESSAGE)                             //Changed here
               ForwardDefault (messageHeader, duplicated,
                              receiverIfaceAddr, inetSourceAddr.GetIpv4 ());
             else
               ForwardTCMessage (messageHeader, duplicated, receiverIfaceAddr, inetSourceAddr.GetIpv4 ());         //Changed here
            }
        }
       
       else //Changed here
         packet->RemoveAtStart (messageHeader.GetSerializedSize ()            //Changed here
                                - messageHeader.GetSerializedSize ());
   }
 }
  // After processing all OLSR messages, we must recompute the routing table
...
/*void Removesinglehop (Ipv4Address neighaddr , singhop & pp) //Changed here
{
   std::set<Ipv4Address> toRemove;
   for (singhop::iterator tt = pp.begin(); tt != pp.end(); tt++)
   {
      if (tt->mainAddr == neighaddr)
      {
        toRemove.insert (tt->mainAddr);
        pp.erase (tt);
      }
   }
  uint8_t i = 0;
  for (singhop::iterator tt = pp.begin(); tt != pp.end(); tt++)
   {
      ...
      if (toRemove.find(tt->mainAddr) != toRemove.end())
      {
         pp.erase (pp.begin() + i);
      }
      else
...
void RoutingProtocol::MprComputation ()
{
  ...
  singhop n;  //Changed here
  NeighborSet s;
  for(NeighborSet::const_iterator i = m_state.GetNeighbors ().begin(); i != m_state.GetNeighbors ().end(); i++)//Changed here
  {
      SingleHop st;
      st.mainAddr = i->neighborMainAddr;
      st.will = i->willingness;
      st.deg = i->degree;
      st.degx2 = rand () % 15;
      uint8_t temp = st.degx2;
      for(TwoHopNeighborSet::iterator ti = m_state.GetTwoHopNeighbors ().begin (); ti != m_state.GetTwoHopNeighbors ().end(); ti++)
      {
         if (ti->neighborMainAddr == i->neighborMainAddr)
         {
            struct TwoHopNeighborTuple *th = new struct TwoHopNeighborTuple;
            th->neighborMainAddr = ti->neighborMainAddr;
            th->twoHopNeighborAddr = ti->twoHopNeighborAddr;
            th->expirationTime = ti->expirationTime;
            th->out_metric = ti->out_metric;
            th->degree = ti->degree;
            
            ti->degree2 = temp;
            th->degree2 = ti->degree2;
            st.stt.push_back (*th);
         }
      } 
      m_state.InsertOneHopTuple (st);
      
  }
  
            
  doublehop n2;  //Changed here
  

  
  for(singhop::iterator ll = n.begin(); ll !=n.end (); ll++)   //Changed here
  {
     for (NeighborSet::const_iterator nn = m_state.GetNeighbors ().begin(); nn !=m_state.GetNeighbors ().end(); nn++) //Changed here
     {
         if(nn->neighborMainAddr == ll->mainAddr)
         {
           for (TwoHopNeighborSet::const_iterator ty = m_state.GetTwoHopNeighbors ().begin (); ty != m_state.GetTwoHopNeighbors ().end(); ty++)
           {
                if(ty->neighborMainAddr == nn->neighborMainAddr)
                {
                    ll->stt.push_back (*ty);
                }
           }
  ...
  for (NeighborSet::iterator ii = m_state.GetNeighbors ().begin(); ii !=m_state.GetNeighbors ().end(); ii++) //Changed here (Willingness 7 means will_always)
  {
    uint8_t r = rand() % 7;
    ii->willingness = r;
  }
  ...
  uint8_t i = 0;
  for (NeighborSet::iterator ii = m_state.GetNeighbors ().begin (); ii !=m_state.GetNeighbors ().end(); ii++)   //Changed here
  {
     if (i%2 == 0)
     {
        ii->degree = 1;
     ...
     {
       for (LinkSet::const_iterator ll = m_state.GetLinks ().begin(); ll != m_state.GetLinks ().end(); ll++)
       {
           if (ll->neighborIfaceAddr == ii->neighborMainAddr)
           {
               ii->degree = ll->mtr;  //assignment of int with enum
           }
       }
     }
     i++;
 ...
 for (TwoHopNeighborSet::iterator it = m_state.GetTwoHopNeighbors ().begin(); it != m_state.GetTwoHopNeighbors ().end(); it++)  //Changed here
  {
     uint8_t count = 0;
     for (NeighborSet::iterator ii = m_state.GetNeighbors ().begin (); ii != m_state.GetNeighbors ().end (); ii++)
     {
        if (it->twoHopNeighborAddr == ii->neighborMainAddr)
        {
            for (LinkSet::const_iterator ll = m_state.GetLinks ().begin (); ll != m_state.GetLinks ().end (); ll++)
            {
                if (ll->localIfaceAddr == it->twoHopNeighborAddr)
                {
                    if (count%2 == 0)
                       it->degree = 1;
                    else
                    {
                        uint8_t current = 0;
                        uint8_t maxi = 35;
                        for (LinkSet::const_iterator ll2 = m_state.GetLinks ().begin (); ll2 != m_state.GetLinks().end (); ll2++)
                        {
                           if (ll2->localIfaceAddr == it->twoHopNeighborAddr)
                           {
                              current = ll2->mtr;
                              if (current<maxi)
                                 maxi = current;
  ...
  for (singhop::iterator i = m_state.GetNeighs ().begin (); i != m_state.GetNeighs ().end (); i++)  //Changed here
  {
       for (TwoHopNeighborSet::iterator p = m_state.GetTwoHopNeighbors ().begin (); p != m_state.GetTwoHopNeighbors ().end (); p++)
       {
            for (allowed2hops::iterator tw = m_state.Getallowed2hops ().begin (); tw != m_state.Getallowed2hops ().end(); tw++)
            {
                 for (LinkSet::const_iterator ll = m_state.GetLinks ().begin (); ll != m_state.GetLinks ().end(); ll++)
                 {
                     if (i->mainAddr == ll->neighborIfaceAddr)
                     {
                         if (tw->neighaddr == ll->neighborIfaceAddr)
                         {
                            if(p->twoHopNeighborAddr == tw->neighaddr)
                            {
                               p->degree2 = p->out_metric;
                               i->stt.push_back (*p);
  ...
  for (singhop::const_iterator x = m_state.GetNeighs().begin(); x!= m_state.GetNeighs().end(); x++)   //CHanged here
  {  
     n.push_back(*x);
  }

  for (singhop::const_iterator p = n.begin(); p != n.end(); )                                         //Changed here
  {
      if (p->will > 2)
      {
         mprSet.insert (p->mainAddr);
         p = n.erase (p);
      }
      else
        p++;
  ...
  uint8_t count = 0;
  for (dxy::const_iterator y = m_state.Getdxy().begin(); y != m_state.Getdxy().end(); y++)             //Changed here
  {
     for (singhop::const_iterator x = m_state.GetNeighs().begin(); x != m_state.GetNeighs().end(); x++)
     {
       for (TwoHopNeighborSet::const_iterator z = m_state.GetTwoHopNeighbors().begin(); z != m_state.GetTwoHopNeighbors().end(); z++)
       {
           if (y->main1 == x->mainAddr && y->main2 == z->twoHopNeighborAddr )
           {
               if (z->degree2 <=10)
               {
                  count++;
      ...
      if (count == 1)
      {
         mprSet.insert (y->main1);
      SingleHop* tuple = m_state.FindSingleHopTuple (y->main1);
       if (tuple == NULL)
       {
           return;
       }
       ...
       {
           RemovalSingleTuple (tuple);
       }  
      }
  }
 
 for (singhop::iterator x = m_state.GetNeighs().begin(); x!= m_state.GetNeighs().end(); x++)          //Changed here1
  {
     uint8_t count = 0;
     for (dxy::iterator y = m_state.Getdxy().begin(); y!= m_state.Getdxy().end(); y++)
     {
        if ((y->d < 100) && (x->mainAddr == y->main1))
        {
            uint8_t min = 200;
            for (singhop::iterator z = m_state.GetNeighs().begin(); z != m_state.GetNeighs().end(); z++)
            {
               for (dxy::iterator v = m_state.Getdxy().begin(); v != m_state.Getdxy().end(); v++)
               {
                   if ((v->main2 == y->main2) && (v->main1 == z->mainAddr) && (mprSet.count(z->mainAddr)==0) )
                   {
                      if (min > v->d)
                      {
                          min = v->d;
    ...
    Rxm r;
    r.mainaddr = x->mainAddr;
    r.count = count;
    m_state.InsertRxmTuple (r);
 }  

  for (singhop::iterator x = m_state.GetNeighs().begin(); x != m_state.GetNeighs().end(); x++)  //Changed here
  {
     for (rxm::const_iterator y = m_state.GetRxm().begin(); y != m_state.GetRxm().end(); y++)
     {
         if ((x->mainAddr == y->mainaddr) && (x->will > 1) && (y->count > 0))
         {
             mprSet.insert (x->mainAddr);
            SingleHop* tuple = m_state.FindSingleHopTuple (x->mainAddr);
            if (tuple == NULL)
            {
                 return;
            }
 ...
 /*NeighborSet N;
  ...
  /*for (NeighborSet::const_iterator nai = N.begin(); nai != N.end(); nai++)            //New Protocol
  {
     if (nai->willingness == OLSR_WILL_NEVER)
     {
        N.erase (nai);
     }
  } */      
      ...
      if (onlyOne)
        {
          ...
          for (NeighborSet::const_iterator neighbor = N.begin(); neighbor != N.end(); neighbor++)      //New Protocol (Removing the nodes in N which have been already put in the MPR set
          {
...
//Uptill this point only the 2hop nodes which are connected by only a single one hop neighbor has been considered. 
 ...
 /*while(N2.begin () != N2.end())
{
   ...
   int prev;
   Ipv4Address ss;
   for (NeighborSet::const_iterator neighbor = N.begin(); neighbor != N.end(); neighbor++)    //New Protocol
   {
       if (neighbor->MPRselectset < min)
       {
          min = neighbor->MPRselectset;
          ss = neighbor->neighborMainAddr;
          prev = Degree (*neighbor);
       }
       else
       {
          if (neighbor->MPRselectset == min)
          {
              if (Degree (*neighbor) > prev)
              {
                 min = neighbor->MPRselectset;
                 ss = neighbor->neighborMainAddr;
                 prev = Degree (*neighbor);
   ...
   for (NeighborSet::const_iterator nai = N.begin(); nai != N.end(); nai++)            //New Protocol
  {
     if (nai->neighborMainAddr == ss)
     {
        N.erase (nai);
     }
  }

  for (TwoHopNeighborSet::const_iterator pp = N2.begin(); pp != N2.end(); pp++)         //New Protocol
  {
      if (pp->neighborMainAddr == ss)
      {
        coveredTwoHopNeighbors.insert (pp->twoHopNeighborAddr);
        for (NeighborSet::const_iterator nei = N.begin(); nei != N.end(); nei++)
        {
            if (nei->neighborMainAddr == pp->twoHopNeighborAddr)
            {
              N.erase (nei);
            }
        }
...
void
RoutingProtocol::RemovalSingleTuple (const SingleHop* tuple)                 //Changed here
{
    m_state.RemoveOneHopTuple (tuple->mainAddr);
}
/*void RoutingProtocol::RemoveSinghopTuple (const SingleHop &tuple)           //Changed here
{
   m_state.RemoveSinglehopTuple (tuple->mainAddr);
}*/
...
void RoutingProtocol::FormARoutingTable (const Ipv4Address &dest, const Ipv4Address &next, const Ipv4Address &interfaceAddr)  //Changed here
{
    LinkTuple *tuple = m_state.FindLinkTuple (dest);
    if (tuple != NULL)
    {
         if (tuple->sll == LinkTuple::SY_M)
         {
              FloodingMPR tuple;
              tuple.neighAddr = dest;
              m_state.InsertFloodingTuple (tuple);
         }
    }
}
...
RoutingProtocol::RoutingTableComputation ()
{
  ...
  const NeighborSet &neighborSet = m_state.GetNeighbors ();                 //Raja Here
      ...
      if (nb_tuple.status == NeighborTuple::STATUS_SYM)
        {
          ...
          for (LinkSet::const_iterator it2 = linkSet.begin ();
               it2 != linkSet.end (); it2++)
            {
              ...
              if ((GetMainAddress (link_tuple.neighborIfaceAddr) == nb_tuple.neighborMainAddr) && (nb_tuple.willingness < 7) && (nb_tuple.N_symmetric == true)   //Changed here
                  && link_tuple.time >= Simulator::Now ())
                {
                 ...
                 FormARoutingTable (link_tuple.neighborIfaceAddr, link_tuple.neighborIfaceAddr, link_tuple.localIfaceAddr);   //Changed here
                  lt = &link_tuple;
                               ...
                               const Ipv4Address &senderIface)
{
  ...
  if (msg.GetMessageTLV () == 1)   //Changed here
  {
           NeighborTuple* x = m_state.FindNeighborTuple (senderIface);
           if(x != NULL)
           {
               RemoveNeighborTuple(*x);
           }
    LinkSet &p = const_cast<LinkSet&>(m_state.GetLinks());
    for (LinkSet::iterator ll = p.begin(); ll !=p.end(); ll++)
    {
        if (ll->neighborIfaceAddr == senderIface)
        {
           ll->mtr = LinkTuple::KN;    
        }
  ...
  LinkTuple* temp = m_state.FindLinkTuple (senderIface);                                  //CHanged here
  if (temp != NULL)
  {
     if ((temp->mtr == LinkTuple::KN) && (m_state.FindMprAddress (temp->localIfaceAddr)))
     {
         FloodingMPR flr;
         flr.neighAddr =  GetMainAddress(senderIface);
         m_state.InsertFloodingTuple (flr);
     }
  }
                            ...
                            const Ipv4Address &senderIface)
{
  ...
  if (tc.GetSerializedSize () != 32)   //Changed here (if the tc message size not equal to the size of addresses used by this router)
  {
      return;
  }

 if (senderIface == NULL || (msg.GetMessageSequenceNumber () < 0 || msg.GetMessageSequenceNumber () >65536) ) //Changed here(if it contains no sequence number)
     return;

  if (msg.GetHopCount () <= 0)  //Changed here
  {
     return;
  }
...
bool RoutingProtocol::ComputeFloodingMPR (const Ipv4Address &senderAddress, const Ipv4Address &receiverAddress, olsr::MessageHeader olsrMessage)
{
      const olsr::MessageHeader::Tc &tc = olsrMessage.GetTc ();
      std::vector<Ipv4Address> pp = tc.mprselectoraddresses;

      for (std::vector<Ipv4Address>::const_iterator it = tc.mprselectoraddresses.begin(); it != tc.mprselectoraddresses.end(); it++)
      {
          if (senderAddress == *it)
          {
              return true;
...
void RoutingProtocol::ForwardTCMessage (olsr::MessageHeader olsrMessage,
                                 DuplicateTuple *duplicated,
                                 const Ipv4Address &localIface,
                                 const Ipv4Address &senderAddress)              //Changed here
{
   Time now = Simulator::Now ();
   bool value = ComputeFloodingMPR (senderAddress, localIface, olsrMessage);
   //const FloodingMPR* tup = m_state.FindFloodingMPR (senderAddress);
   ...
   if (duplicated != NULL && duplicated->retransmitted)
    ...
    if (olsrMessage.GetTimeToLive () > 1)
    {
         //const FloodingMPR* tp = m_state.FindFloodingMPR (senderAddress);
         if (value == true)
         {
             olsrMessage.SetTimeToLive (olsrMessage.GetTimeToLive () - 1);
          olsrMessage.SetHopCount (olsrMessage.GetHopCount () + 1);
...
RoutingProtocol::SendHello ()
{
  ...
  /*int count = 0;
  for (MprSelectorSet::const_iterator it = m_state.GetMprSelectors ().begin (); it != m_state.GetMprSelectors ().end(); it++) //New Protocol
  {
     count++;
  }*/
  //msg.SetMPRselectsize (count);                //New Protocol
  msg.SetMessageTLV (1);    //Changed here
  msg.SetNumTLV (1);        //Changed here
...
RoutingProtocol::SendTc ()
{
  ...
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
  msg.SetMessageTLV (2);    //Changed here
  msg.SetNumTLV (1);        //Changed here
//
  olsr::MessageHeader::Tc &tc = msg.GetTc ();
  tc.ansn = m_ansn;

  for (mfsp::const_iterator mprflood = m_state.GetFloodingMPRselectors ().begin (); mprflood != m_state.GetFloodingMPRselectors ().end (); mprflood++)   //Changed here
  {
      tc.mprselectoraddresses.push_back (mprflood->neighAddr);
  }
...
RoutingProtocol::SendMid ()
{
  ...
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
  msg.SetMessageTLV (3);    //Changed here
  msg.SetNumTLV (1);        //Changed here
...
RoutingProtocol::SendHna ()
{
  ...
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
  msg.SetMessageTLV (4);    //Changed here
  msg.SetNumTLV (1);        //Changed here
                                      ...
                                      const olsr::MessageHeader::Hello &hello)
{
  ...
  if (nb_tuple != NULL)
    {
      NeighborTuple temp;                                           //CHanged here
      temp.neighborMainAddr = nb_tuple->neighborMainAddr;
      temp.status = nb_tuple->status;
      temp.willingness = nb_tuple->willingness;
      temp.degree = nb_tuple->degree;                     //Changed here
      temp.N_symmetric = nb_tuple->N_symmetric;             //Changed here*/
      nb_tuple->willingness = hello.willingness;
      //nb_tuple->MPRselectset = msg.GetMPRselectsize ();                         //New Protocol
      
      bool check = m_state.FindNSymmetric (temp);            //Changed here

      if (check)                //Changed here
      {
         LinkTuple* ll = m_state.FindLinkTuple (temp.neighborMainAddr);   
         if (ll != NULL)
         {
            m_state.ChangeCorrespondingOutMetric (ll->localIfaceAddr);
         }
      }
    }
