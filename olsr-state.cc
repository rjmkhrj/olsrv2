...
bool OlsrState::FindNSymmetric (const NeighborTuple &tuple)            //CHanged here
{
    if (tuple.N_symmetric == true)
      return true;
    else 
      return false;
}
...
void OlsrState::InsertOneHopTuple (SingleHop const &tuple)                //Changed here
{
  for (singhop::iterator it = m_neighset.begin(); it != m_neighset.end(); it++)
  {
     if(it->mainAddr == tuple.mainAddr)
     {
        *it = tuple;
         return;
     }
   ...
   m_neighset.push_back (tuple);
}

void OlsrState::RemoveOneHopTuple (const Ipv4Address &mainAddr)             //Changed here
{
   for (singhop::iterator it = m_neighset.begin(); it!= m_neighset.end(); it++)
   {
      if(it->mainAddr == mainAddr)
      {
         it = m_neighset.erase (it);
         break;
      }
    }
}

SingleHop* OlsrState::FindSingleHopTuple (Ipv4Address const &mainAdd)
{
   for (singhop::iterator it = m_neighset.begin(); it != m_neighset.end(); it++)
   {
     if (it->mainAddr == mainAdd)
     {
       return &(*it);
     }
   }
...
/******************************Rxm tuple****************************/  //Changed here

void OlsrState::InsertRxmTuple (Rxm const &tuple)
{
   for (rxm::iterator it = m_rxm.begin(); it != m_rxm.end(); it++)
   {
      if (it->mainaddr == tuple.mainaddr)
      {
         *it = tuple;
   ...
   m_rxm.push_back (tuple);
}


/******************************dxy tuple****************************/

void OlsrState::InsertdxyTuple (Comb const &tuple)                            //Changed here 
{
   for (dxy::iterator it = m_dxy.begin(); it != m_dxy.end(); it++)
   {
      if ((it->main1 == tuple.main1) && (it->main2 == tuple.main2))
      {
          *it = tuple;
          return;
      }
   }
   m_dxy.push_back (tuple);
} 

/*********************************DoubleHopN***********************/

void OlsrState::InsertNtuple (DoubleHopN const &tuple)
{
    for (doublehop::iterator it = m_doubleset.begin(); it != m_doubleset.end(); it++)
    {
        if (it->mainAddr == tuple.mainAddr)
        {
            *it = tuple;
     ...
     m_doubleset.push_back (tuple);
}


/********** Neighbor 2 Hop Set Manipulation **********/

TwoHopNeighborTuple* OlsrState::FindTwoHopNeighborTuple (Ipv4Address const &neighborMainAddr,
...
void OlsrState::ChangeCorrespondingOutMetric (const Ipv4Address &t)             //Changed here
{
   for (LinkSet::iterator it = m_linkSet.begin(); it != m_linkSet.end(); it++)
   {
       if (it->localIfaceAddr == t)
       {
           it->mtr = LinkTuple::KN;
           break;
       }
...
/*********************Flooding Tuple *******************************/ //Changed here

void OlsrState::InsertFloodingTuple (const FloodingMPR &tuple)
{
   m_fmpr.push_back (tuple);
}

FloodingMPR* OlsrState::FindFloodingMPR (const Ipv4Address &addr)
{
   for (fmpr::iterator it = m_fmpr.begin(); it != m_fmpr.end(); it++)
   {
       if (it->neighAddr == addr)
       {
           return &(*it);
       }
   }
...
/********************MPRFloodingSelector*************************/ //Changed here


void OlsrState::InsertFloodingMPRSelector (const MPRFloodingSelector &tuple)               //CHanged here
{
   m_mfsp.push_back (tuple);
}

MPRFloodingSelector* OlsrState::FindMPRFloodingSelector (const Ipv4Address &addr)      //Changed here
{
   for (mfsp::iterator it = m_mfsp.begin(); it != m_mfsp.end(); it++)
   {
       if (it->neighAddr == addr)
       {
           return &(*it);
...
/*********************Per Interface Tuple****************************/  //Changed here

InterfaceTuple* OlsrState::FindInterfaceTuple (const Ipv4Address &senderAddr, const uint16_t &seqNo, const MessageType &mg)  //Changed here
{
   for (InterfaceSet::iterator it = m_interfaces.begin(); it != m_interfaces.end(); it++)
   {
      if (it->senderAddress == senderAddr and it->seqNumber == seqNo and it->mg == mg)
        {
           return &(*it);
        }
   }
...
void OlsrState::InsertInterfaceTuple (const InterfaceTuple &tuple) //Changed here
{
   m_interfaces.push_back (tuple);
}


/********************Processed Tuple Set****************************/ //Changed here

ProcessedTuple* OlsrState::FindProcessedTuple (const Ipv4Address &senderAddr, const uint16_t &seqNo, const MessageType &mg) //Changed here
{
   for (ProcessedSet::iterator it = m_processed.begin(); it != m_processed.end(); it++)
   {
      if (it->senderAddress == senderAddr and it->seqNumber == seqNo and it->mg == mg)
         {
            return &(*it);
...
void OlsrState::InsertProcessedTuple (const ProcessedTuple &tuple)  //Changed here
{
   m_processed.push_back (tuple);
}
}
}  // namespace olsr, ns3
